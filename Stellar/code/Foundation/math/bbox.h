...
{
    bool lt = float4::any(float4::less(this->pmax, box.pmin));
    bool gt = float4::any(float4::greater(this->pmin, box.pmax));
    return !(lt || gt);
}
...
{
    bool lt = float4::all(float4::less(this->pmin, box.pmin));
    bool ge = float4::all(float4::greaterequal(this->pmax, box.pmax));
    return lt && ge;
}
...
{
    bool lt = float4::all(float4::less(this->pmin, p));
    bool ge = float4::all(float4::greaterequal(this->pmax, p));
    return lt && ge;
}
