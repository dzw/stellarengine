...
Win32FSWrapper::OpenFile(const String& path, Stream::AccessMode accessMode, Stream::AccessPattern accessPattern)
{
    ...
    Handle handle = CreateFile(path.c_str(),        // lpFileName
...
Win32FSWrapper::Write(Handle handle, const void* buf, Stream::Size numBytes)
{
    ...
    {
        s_error("Win32FSWrapper: WriteFile() failed with '%s'", Win32::SysFunc::GetLastError().c_str());
    }
}
...
Win32FSWrapper::Read(Handle handle, void* buf, Stream::Size numBytes)
{
    ...
    if (0 == result)
    {
        s_error("Win32FSWrapper: ReadFile() failed with '%s'", Win32::SysFunc::GetLastError().c_str());
    }
    return bytesRead;
}
...
{
    s_assert(!path.empty());
    return (0 != ::DeleteFileA(path.c_str()));
}
...
{
    s_assert(!path.empty());
    return (0 != ::RemoveDirectory(path.c_str()));
}
...
Win32FSWrapper::GetFileWriteTime(const String& path)
{
    ...
    {
        s_error("Win32FSWrapper::GetFileWriteTime(): failed to open file '%s'!", path.c_str());
    }
    return fileTime;
}
...
{
    s_assert(!dirPath.empty());
    s_assert(!pattern.empty());
...
//    if (n_stricmp(pathToDir.c_str(), "win32") == 0)
//    {
