...
{
    s_error("sphere::intersects(): NOT IMPLEMENTED!");
    return false;
/*
    float s, d = 0;
...
{
    s_error("sphere::intersect_sweep(): NOT IMPLEMENTED!");
    return false;
...
{
    s_error("sphere::project_screen_rh(): NOT IMPLEMENTED!");
    return rectangle<scalar>(0, 0, 0, 0);
/*
    // compute center point of the sphere in view space
    vector viewPos = view * this->p;
    if (viewPos.z > -nearZ)
    {
        viewPos.z = -nearZ;
    }
    vector screenPos  = projection.mult_divw(viewPos);
    ...
    vector screenSize = projection.mult_divw(vector(this->r, this->r, frontZ));
    screenSize.y = -screenSize.y;        
    float left   = s_saturate(0.5f * (1.0f + (screenPos.x - screenSize.x)));
    float right  = s_saturate(0.5f * (1.0f + (screenPos.x + screenSize.x)));
    float top    = s_saturate(0.5f * (1.0f + (screenPos.y + screenSize.y)));
    float bottom = s_saturate(0.5f * (1.0f + (screenPos.y - screenSize.y)));
