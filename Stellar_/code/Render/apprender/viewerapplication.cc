...
ViewerApplication::Open()
{
    ...
    if (RenderApplication::Open())
    {
        ...
        this->view = gfxServer->CreateView(View::RTTI, StringAtom("DefaultView"));
        this->view->SetStage(this->stage);
        this->view->SetFrameShader(FrameServer::Instance()->GetFrameShaderByName(ResourceId("DX9Default")));
...
ViewerApplication::OnProcessInput()
{
    ...
    else
    {
        ...
        if (gamePad->IsConnected())
        {
            ...
            zoomIn       += gamePad->GetAxisValue(GamePad::RightTriggerAxis);
            zoomOut      += gamePad->GetAxisValue(GamePad::LeftTriggerAxis);
            panning.x()  += gamePad->GetAxisValue(GamePad::RightThumbXAxis);
            panning.y()  += gamePad->GetAxisValue(GamePad::RightThumbYAxis);
            orbiting.x() += gamePad->GetAxisValue(GamePad::LeftThumbXAxis);
            orbiting.y() += gamePad->GetAxisValue(GamePad::LeftThumbYAxis);
        }
...
{
    // @todo: hmm, this is too granular??
    this->stage->BeginUpdate();
    this->stage->UpdateEntities();
    this->view->UpdateVisibilityLinks();
    this->stage->UpdateLightLinks();
    this->stage->EndUpdate();
    this->view->Render();
