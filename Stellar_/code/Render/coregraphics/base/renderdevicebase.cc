...
RenderDeviceBase::~RenderDeviceBase()
{
    ...
    s_assert(this->eventHandlers.IsEmpty());
    DestructSingleton;
}
...
RenderDeviceBase::Open()
{
    ...
    s_assert(!this->defaultRenderTarget.isvalid());
    this->isOpen = true;
...
{
    s_assert(handler.isvalid());
    s_assert(InvalidIndex == this->eventHandlers.FindIndex(handler));
    s_assert(!this->inNotifyEventHandlers);
...
RenderDeviceBase::NotifyEventHandlers(const RenderEvent& event)
{
    ...
    {
        handled |= this->eventHandlers[i]->OnEvent(event);
    }
...
RenderDeviceBase::BeginFrame()
{
    ...
    s_assert(!this->inBeginPass);
    s_assert(!this->inBeginBatch);
    s_assert(!this->vertexBuffer.isvalid());
    s_assert(!this->indexBuffer.isvalid());
...
RenderDeviceBase::BeginPass(const Ptr<RenderTarget>& rt, const Ptr<ShaderInstance>& shd)
{
    ...
    s_assert(!this->inBeginPass);
    s_assert(!this->inBeginBatch);
    s_assert(!this->passShader.isvalid());
...
RenderDeviceBase::BeginBatch(BatchType::Code batchType, const Ptr<ShaderInstance>& shd)
{
    s_assert(this->inBeginPass);
    s_assert(!this->inBeginBatch);
    s_assert(!this->batchShader.isvalid());
    s_assert(this->passRenderTarget.isvalid());
    s_assert(shd.isvalid());
