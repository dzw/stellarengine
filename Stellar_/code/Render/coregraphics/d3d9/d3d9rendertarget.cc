...
D3D9RenderTarget::~D3D9RenderTarget()
{
    ...
    s_assert(0 == this->d3d9RenderTarget);
    s_assert(0 == this->d3d9DepthStencil);
    s_assert(0 == this->d3d9ResolveTexture);
}
...
{
    s_assert(D3DFMT_UNKNOWN != this->d3d9ColorBufferFormat);
    ...
    IDirect3D9* d3d9 = renderDevice->GetDirect3D();
        ...
        {
            s_assert(SUCCEEDED(renderTargetResult) && SUCCEEDED(depthBufferResult));
        }
...
D3D9RenderTarget::BeginPass()
{
    ...
    {
        s_assert(this->IsDefaultRenderTarget());
