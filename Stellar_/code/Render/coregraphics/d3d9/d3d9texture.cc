...
D3D9Texture::~D3D9Texture()
{
    s_assert(0 == this->d3d9BaseTexture);
    s_assert(0 == this->d3d9Texture);
    s_assert(0 == this->d3d9CubeTexture);
    s_assert(0 == this->d3d9VolumeTexture);
...
{
    s_assert((this->type == Texture2D) || (this->type == Texture3D));
    s_assert(MapWriteNoOverwrite != mapType);
    bool retval = false;
...
D3D9Texture::Unmap(IndexT mipLevel)
{
    ...
    s_assert((Texture2D == this->type) || (Texture3D == this->type));
    if (Texture2D == this->type)
    {
        s_assert(0 != this->d3d9Texture);
...
{
    s_assert(TextureCube == this->type);
    s_assert(0 != this->d3d9CubeTexture);
    s_assert(MapWriteNoOverwrite != mapType);
    DWORD lockFlags = D3DLOCK_NO_DIRTY_UPDATE | D3DLOCK_NOSYSLOCK;
    switch (mapType)
