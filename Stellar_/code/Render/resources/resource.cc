...
namespace Resources
{
ImplementClass(Resources::Resource, 'RSRC', Core::RefCounted);
...
Resource::~Resource()    
{
    s_assert(!this->IsLoaded());
    s_assert(!this->IsPending());
    s_assert(!this->loader.isvalid());
    //s_assert(!this->saver.isvalid());
}
...
Resource::Load()
{
    ...
    {
        s_error("Failed to load resource '%d'!\n", this->resourceId);
    }
    return this->state;
}
