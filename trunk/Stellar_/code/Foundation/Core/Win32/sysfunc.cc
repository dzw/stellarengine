...
SysFunc::Error(const char* error)
{
    ...
    #endif
    HWND hwnd = FindWindow("StellarMain", NULL);
    if (hwnd)
    {
        ShowWindow(hwnd, SW_MINIMIZE);
    }
    MessageBox(NULL, error, "STELLAR SYSTEM ERROR", MB_OK|MB_APPLMODAL|MB_SETFOREGROUND|MB_TOPMOST|MB_ICONERROR);
