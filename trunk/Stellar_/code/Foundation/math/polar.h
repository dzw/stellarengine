...
polar::set(const vector& vec)
{
    ...
    {
        // quadrant 1
        this->rho = s_asin(normVec2d.x());
    }
    else if ((normVec2d.x() < 0.0f) && (normVec2d.y() >= 0.0f))
    {
        // quadrant 2 
        this->rho = s_asin(normVec2d.y()) + s_deg2rad(270.0f);
    }
    else if ((normVec2d.x() < 0.0f) && (normVec2d.y() < 0.0f))
    {
        // quadrant 3
        this->rho = s_asin(-normVec2d.x()) + s_deg2rad(180.0f);
    }
    else
    {
        // quadrant 4
        this->rho = s_asin(-normVec2d.y()) + s_deg2rad(90.0f);
    }
}
...
{
    scalar sinTheta = s_sin(this->theta);
    scalar cosTheta = s_cos(this->theta);
    scalar sinRho   = s_sin(this->rho);
    scalar cosRho   = s_cos(this->rho);
