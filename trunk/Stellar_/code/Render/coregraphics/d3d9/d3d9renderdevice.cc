...
{
    s_assert(0 != this->d3d9);

    DisplayDevice* displayDevice = DisplayDevice::Instance();
    Adapter::Code requestedAdapter = displayDevice->GetAdapter();
    s_assert(displayDevice->AdapterExists(requestedAdapter));
...
D3D9RenderDevice::SetupBufferFormats()
{
    ...
    if (FAILED(hr))
    {
        s_error("Rendering device doesn't support D24S8 depth buffer!\n");
        return;
    }
    ...
    if (FAILED(hr))
    {
        s_error("Backbuffer format not compatible with D24S8 depth buffer!\n");
        return;
    }
...
D3D9RenderDevice::OpenDirect3DDevice()
{
    ...
    if (FAILED(hr))
    {
        s_error("Failed to create Direct3D device object: %s!\n", DXGetErrorString9(hr));
        return false;
    }
...
D3D9RenderDevice::CloseDirect3DDevice()
{
    ...
    this->d3d9Device->SetStreamSource(0, NULL, 0, 0);
    IndexT i;
    for (i = 0; i < 8; i++)
    {
...
D3D9RenderDevice::EndFrame()
{
    ...
    // clear currently set resources
    hr = this->d3d9Device->SetStreamSource(0, NULL, 0, 0);
    s_assert(SUCCEEDED(hr));
    hr = this->d3d9Device->SetIndices(NULL);
    s_assert(SUCCEEDED(hr));
}
...
*/
void
D3D9RenderDevice::SetVertexBuffer(const Ptr<VertexBuffer>& vb)
{
    s_assert(this->inBeginPass);
    s_assert(0 != this->d3d9Device);    
    s_assert(vb.isvalid());

    if (this->vertexBuffer != vb)
    {
        HRESULT hr;

        // set vertex buffer on d3d device
        const Ptr<VertexLayout>& vertexLayout = vb->GetVertexLayout();
        IDirect3DVertexBuffer9* d3d9VertexBuffer = vb->GetD3D9VertexBuffer();
        hr = this->d3d9Device->SetStreamSource(0, d3d9VertexBuffer, 0, vertexLayout->GetVertexByteSize());
        s_assert(SUCCEEDED(hr));

        // set vertex declaration
        IDirect3DVertexDeclaration9* d3d9VertexDecl = vertexLayout->GetD3D9VertexDeclaration();
        hr = this->d3d9Device->SetVertexDeclaration(d3d9VertexDecl);
        s_assert(SUCCEEDED(hr));
    }
    RenderDeviceBase::SetVertexBuffer(vb);
}
