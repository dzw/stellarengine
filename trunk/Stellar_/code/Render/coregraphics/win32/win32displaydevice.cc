...
Win32DisplayDevice::Win32DisplayDevice() :
    ...
    windowClassName("Nebula3 Window Class")
{
    ConstructSingleton;
...
/**
    ...
    button, or hits Alt-F4, the method IsQuitRequested() will return
    true after this method returns. If you want to intercept Alt-F4 (for
    instance to display a "Really Quit" message box, attach an event handler
    to the display device and listen for the CloseRequested event.
*/
void
Win32DisplayDevice::ProcessWindowMessages()
{
    s_assert(this->IsOpen());
    s_assert(0 != this->hWnd);
    s_assert(0 != this->hAccel);
    MSG msg;
...
Win32DisplayDevice::OpenWindow()
{
    ...
    HICON icon = 0;
    if (!this->iconName.empty())
    {
        icon = LoadIcon(this->hInst, this->iconName.c_str());
    }
    ...
    // open window
    this->hWnd = CreateWindow(this->windowClassName.c_str(),    // lpClassName
                              this->windowTitle.c_str(),        // lpWindowName
                              windowStyle,                          // dwStyle
                              adjMode.GetXPos(),                    // x
                              adjMode.GetYPos(),                    // y
...
Win32DisplayDevice::CloseWindow()
{
    ...
    UnregisterClass(this->windowClassName.c_str(), this->hInst);
}
...
Win32DisplayDevice::ComputeAdjustedWindowRect()
{
    ...
    else
    {
        ...
        return DisplayMode(r.left, r.top, r.right - r.left, r.bottom - r.top);
    }
}
...
{
    return true;//this->NotifyEventHandlers(DisplayEvent(DisplayEvent::SetCursor));
}
...
Win32DisplayDevice::ComputeNormMousePos(const float2& absMousePos) const
{
    ...
    {
        LONG w = s_max(clientRect.right - clientRect.left, 1);
        LONG h = s_max(clientRect.bottom - clientRect.top, 1);
