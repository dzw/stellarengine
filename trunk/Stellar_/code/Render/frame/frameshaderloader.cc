...
FrameShaderLoader::LoadFrameShaders(const URI& uri)
{
    ...
    if (xmlReader->Open())
    {
        ...
        {
            s_error("FrameShaderLoader: '%s' is not a valid frame shader!", uri.AsString().c_str());
            return result;
        }
    ...
    {
        s_error("FrameShaderLoader: failed to load frame shader '%s'!", uri.AsString().c_str());
    }
    return result;
}
...
{
    s_assert(DisplayDevice::Instance()->IsOpen());
...
FrameShaderLoader::ParseShaderVariableInstance(const Ptr<XmlReader>& xmlReader, const Ptr<ShaderInstance>& shd)
{
    ...
    {
        s_error("FrameShaderLoader: shader '%s' has no variable '%s' (%s, line %d)",
            shd->GetOriginalShader()->GetResourceId().Value().c_str(),
            semantic.c_str(),
            xmlReader->GetStream()->GetURI().AsString().c_str(),
...
FrameShaderLoader::ParseFramePass(const Ptr<XmlReader>& xmlReader, const Ptr<FrameShader>& frameShader)
{
    ...
    if (xmlReader->HasAttr("renderTarget"))
    {
        ...
        {
            s_error("FrameShaderLoader: render target '%s' not declared (%s, line %d)",
                xmlReader->GetStream()->GetURI().AsString().c_str(), 
                xmlReader->GetCurrentNodeLineNumber());
        }
    }
